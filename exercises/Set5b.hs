...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node r left right) = 1 + treeSize left + treeSize right
...
treeMax Empty = 0
treeMax (Node r left right) = max r (max (treeMax left) (treeMax right))
...
walk (x:xs) (Node root left right) = if x == StepL then walk xs left else walk xs right
walk [] Empty = Nothing
walk [] (Node root _ _) = Just root
...
search val (Node root left right) = if root == val then Just [] else case search val left of 
                                                                        Just ys -> Just (StepL : ys)
