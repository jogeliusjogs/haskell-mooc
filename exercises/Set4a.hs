...
longest :: (Ord a) => [[a]] -> [a]
longest xs = head (sortBy longestListWithSmallestFirstElement xs)

longestListWithSmallestFirstElement :: (Ord a) => [a] -> [a] -> Ordering
longestListWithSmallestFirstElement x y
  | length x > length y = LT
  | length x < length y = GT
  | length x == length y = compare (head x) (head y)
...
freqs xs = go Map.empty xs
  where go currMap (x:[]) = case (Map.lookup x currMap) of
                              Just val -> Map.insert x (val+1) currMap
                              Nothing -> Map.insert x 1 currMap
        go currMap (x:xs) = case (Map.lookup x currMap) of
                              Just val -> go (Map.insert x (val+1) currMap) xs
                              Nothing -> go (Map.insert x 1 currMap) xs
...
transfer from to amount bank = case Map.lookup from bank of
  Nothing -> bank
  Just fromLookup -> case Map.lookup to bank of
    Nothing -> bank
    Just toLookup -> if amount > 0 && fromLookup >= amount
      then Map.insert to (toLookup + amount) (Map.insert from (fromLookup - amount) bank)
      else bank
