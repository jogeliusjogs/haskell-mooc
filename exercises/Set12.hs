...
count element xs = foldr (\y acc -> if element == y then (acc+1) else acc) 0 xs
...
instance Foldable TwoList where
  ...
  foldr f acc (TwoNode a b l) = f a (f b (foldr f acc l))
...
instance Functor Tree where
  fmap f Leaf = Leaf
  fmap f (Node n ta tb) = Node (f n) (fmap f ta) (fmap f tb)

sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node n ta tb) =  (sumTree ta) <> n <> (sumTree tb)
