...
get :: UpDown -> Int
get (UpDownIncreasing count) = count
get (UpDownDecreasing count) = count

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (UpDownIncreasing count) = UpDownIncreasing (count+1)
tick (UpDownDecreasing count) = UpDownDecreasing (count-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpDownIncreasing count) = UpDownDecreasing count
toggle (UpDownDecreasing count) = UpDownIncreasing count
...
rgb col = case col of
  ...
  Green -> [0, 1, 0]
  Blue -> [0, 0, 1]
  Mix a b -> [(head (rgb a) + head (rgb b)) / 2, (rgb a !! 1 + rgb b !! 1) / 2, (rgb a !! 2 + rgb b !! 2) / 2]
  Invert a -> [1 - head (rgb a), 1 - rgb a !! 1, 1 - rgb a !! 2]
...
toNat n 
  | n < 0 = Nothing
  | n == 0 = Just Zero
  | n > 0 = Just (PlusOne (case toNat (n-1) of
    Nothing -> Zero
    Just x -> x))
...
fromBin b = fromBinHelper 0 b
  where fromBinHelper powerAcc b = case b of
                                      End -> 0
                                      I b -> 2^powerAcc + fromBinHelper (powerAcc+1) b
                                      O b -> fromBinHelper (powerAcc+1) b
