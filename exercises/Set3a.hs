...
joinToLength l inputs = filter (\x -> length x == l) [first ++ second | first <- inputs, second <- inputs]
...
sumRights [] = 0
sumRights (x:xs) = sumRightsHelper x + sumRights xs

sumRightsHelper:: Either a Int -> Int
sumRightsHelper x = case x of
                      Left l -> 0
                      Right r -> r
